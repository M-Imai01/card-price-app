...
function collectStores() {
  return getStoreRows()
    ...
    .filter((x) => x.shopName && Number.isFinite(x.highestPrice) && x.highestPrice >= 0)
}

function computeSummary(stores, appraisalPrice) {
  const maxStorePrice = stores.length ? Math.max(...stores.map((s) => Number(s.highestPrice || 0))) : 0
...
function buildCardHtml(card) {
  const stores = [...(card.card_shop_prices || [])].sort((a, b) => Number(b.highest_price) - Number(a.highest_price))
  const { maxStorePrice, diffPrice } = computeSummary(stores, card.appraisal_price)
  const publicUrl = supabase.storage.from(BUCKET).getPublicUrl(card.image_path).data.publicUrl
...
async function deleteCard(cardId, imagePath) {
  ...
  try {
    const { error: fileError } = await supabase.storage.from(BUCKET).remove([imagePath])
    if (fileError) {
